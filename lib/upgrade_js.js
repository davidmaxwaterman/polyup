...
// Turn on all ecma features to make the parser as lenient as possible.
// We don't yet support transforming all Polymer patterns when expressed in
// ES6 style, but we shouldn't choke on ES6 code.
const ecmaFeatures = {
...
class Script {
  constructor(jsSource, elements, initialIndent, implicitElemName, onlyReformat) {
  ...
  upgrade() {
    let ast = espree.parse(this.jsSource, {attachComment: true, ecmaFeatures});
    ...
    if (this.onlyReformat) {
      ast = espree.parse(this.jsSource, {attachComment: true, ecmaFeatures});
      removeDuplicateComments(ast);
    }
  ...
  upgradePolymerCall(polyCall) {
    ...
    migrateAttributesToPropertiesBlock(
        polyCall, declaration, attrs, newDeclarations);
...
function upgradeJs(jsSource, elements, implicitElemName=null, initialIndent=0, onlyReformat=false) {
...
function migrateAttributesToPropertiesBlock(
  ...
  declaration.properties.forEach(function(property) {
    ...
    if (property && property.leadingComments) {
      attr.leadingComments = attr.leadingComments.concat(
          property.leadingComments);
    }
    if (literalTypes.has(property.value.type)) {
      attr.value = property.value;
    } else {
      attr.value = {type: 'FunctionExpression', params: [], body: {
        type: 'BlockStatement', body: [{
          type: 'ReturnStatement', argument: property.value}]
      }};
    }
  });
  ...
  declaration.properties.forEach(function(property) {
    ...
    property.value.properties.forEach(function(publishedProp) {
      let attrName = getPropertyKeyName(publishedProp);
      let attr = attrs[attrName] || {name: attrName};
  ...
  declaration.properties.forEach(function(prop) {
    let propName = getPropertyKeyName(prop);
    if (!propName.endsWith('Changed')) {
      return;
    }
    let attrName = propName.substring(0, propName.length - 7);
    attrs[attrName] = attrs[attrName] || {name: attrName};
    attrs[attrName].observer = {type: 'Literal', value: propName};
    fixObserverArgumentOrder(prop.value);
  });

  let observerFunctionNames = new Set();
  declaration.properties.forEach(function(prop) {
    ...
    if (prop.value.type !== 'ObjectExpression') {
      throw new Error(
          'Expected the "observe" field on ' + getPropertyKeyName(prop) +
    ...
    prop.value.properties.forEach(function(observedProp) {
      let propName = getPropertyKeyName(observedProp);
      let attrNames = propName.split(/\s+/);
      ...
      let observerFunctionName = observedProp.value.value;
  ...
  declaration.properties.forEach(function(prop) {
    ...
    if (prop.value.type !== 'FunctionExpression') {
      throw new Error(
          ...
          propName + " would be a function, not a " + prop.value.type);
    }
    fixObserverArgumentOrder(prop.value);
  });
  ...
  declaration.properties.forEach(function(prop) {
    ...
    prop.value.properties.forEach(function(computedProp) {
      ...
      const initialName = 'compute' + computedPropName.charAt(0).toUpperCase() +
...
function fixupComputedExpression(expression) {
  ...
  let declarationOfExpressionFunction = {
    type: 'Property',
  ...
  if (expressionFunctionCache.has(key)) {
    ...
    declarationOfExpressionFunction.rename = function(name) {
      for (let onRename of this.onRenameHandlers) {
        onRename(name);
      }
    };
  }
